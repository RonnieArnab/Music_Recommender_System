...
song_cluster_pipeline = pickle.load(open("./models/song_cluster_pipeline.pkl", "rb"))
...
def get_track_poster_image_url(song, year):
    results = sp.search(q="track:{} year:{}".format(song, year), limit=1)
    ...
    poster_image_url = (
        album_details["images"][0]["url"] if album_details["images"] else None
    )
